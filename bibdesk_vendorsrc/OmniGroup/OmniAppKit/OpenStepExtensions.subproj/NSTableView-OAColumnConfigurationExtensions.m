...
RCS_ID("$Header: /Network/Source/CVS/OmniGroup/Frameworks/OmniAppKit/OpenStepExtensions.subproj/NSTableView-OAColumnConfigurationExtensions.m,v 1.9 2004/02/10 04:07:34 kc Exp $")
...
+ (void)didLoad;
{
    originalOutlineSetDataSource = OBReplaceMethodImplementationWithSelector(self, @selector(setDataSource:), @selector(_replacementSetDataSource:));
    originalOutlineReloadData = OBReplaceMethodImplementationWithSelector(self, @selector(reloadData), @selector(_replacementReloadData));
}
...
- (void)_setupColumnsAndMenu;
{
    ...
    if ([self _allowsAutoresizing]) {
        menuItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedStringFromTableInBundle(@"Auto Size Column", @"OmniAppKit", [isa bundle], "autosize column contextual menu item") action:@selector(autosizeColumn:) keyEquivalent:@""];
        [menuItem setTarget:self];
        [columnsMenu addItem:menuItem];
        [menuItem release];
    
        menuItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedStringFromTableInBundle(@"Auto Size All Columns", @"OmniAppKit", [isa bundle], "autosize all columns contextual menu item") action:@selector(autosizeAllColumns:) keyEquivalent:@""];
        [menuItem setTarget:self];
        [columnsMenu addItem:menuItem];
        [menuItem release];
