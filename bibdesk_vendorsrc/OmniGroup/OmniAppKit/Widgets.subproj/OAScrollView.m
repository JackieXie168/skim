...
{
    flags.smoothScrollDisabled = smoothScrollEnabled? 0 : 1;
}

- (BOOL)smoothScrollEnabled;
{
    return flags.smoothScrollDisabled? NO : YES;
}
...
#if 0
#warning We MUST look at the difference in visible rects here and if small enough just call super.  This is because this method gets called for when we are using fine controls like the scroller knob or buttons.
#warning Alternatively, if the difference in rects is on the order of the scroll quanta, we could use a much lower time interval, or just scroll one pixel at a time.

- (void)scrollClipView:(NSClipView *)aClipView toPoint:(NSPoint)aPoint;
{
    NSRect oldVisibleRect, newVisibleRect, overlapRect, newImageRect;
    NSBitmapImageRep *oldImageRep, *newImageRep;
    NSView *documentView;
    OAImageSlideDirection direction;
    
    if (flags.smoothScrollDisabled) {
        [super scrollClipView:aClipView toPoint:aPoint];
        return;
    }
    
    oldVisibleRect = [self documentVisibleRect];

    // Figure out what direction we are moving.  If we are moving less
    // than 10% of the length of the visible region along that axis,
    // we won't bother doing smooth scrolling.
    
    if (oldVisibleRect.origin.x == aPoint.x) {
        if (fabs(oldVisibleRect.origin.y - aPoint.y / oldVisibleRect.size.height) < 0.10) {
            [super scrollClipView: aClipView toPoint: aPoint];
            return;
        }
        
        if (oldVisibleRect.origin.y < aPoint.y) {
            direction = OAUpSlideDirection;
        } else
            direction = OADownSlideDirection;
    } else if (oldVisibleRect.origin.y == aPoint.y) {
        if (fabs(oldVisibleRect.origin.x - aPoint.x / oldVisibleRect.size.width) < 0.10) {
            [super scrollClipView: aClipView toPoint: aPoint];
            return;
        }

        if (oldVisibleRect.origin.x < aPoint.x) {
            direction = OALeftSlideDirection;
        } else
            direction = OARightSlideDirection;
    } else {
        // We're going diagonal or something ... just pop for now.
        [super scrollClipView: aClipView toPoint: aPoint];
        return;
    }
    
    documentView = [self documentView];
    //NSLog(@"oldVisibleRect = %@", NSStringFromRect(oldVisibleRect));
    
    // Cache the old image
    oldImageRep = [documentView bitmapForRect: oldVisibleRect];
    //[[oldImageRep TIFFRepresentation] writeToFile: @"/tmp/old.tiff" atomically: YES];
    
    // Do the scroll
    [super scrollClipView: aClipView toPoint: aPoint];
    
    // Cache the new image.  If the two rects overlap, we only want to cache
    // the non-overlapping part of the new image.
    newVisibleRect = [self documentVisibleRect];
    //NSLog(@"newVisibleRect = %@", NSStringFromRect(newVisibleRect));

    overlapRect = NSIntersectionRect(oldVisibleRect, newVisibleRect);
    //NSLog(@"overlapRect = %@", NSStringFromRect(overlapRect));

    switch (direction) {
        case OALeftSlideDirection:
            newImageRect = newVisibleRect;
            newImageRect.origin.x += NSWidth(overlapRect);
            newImageRect.size.width -= NSWidth(overlapRect);
            break;
        ...
        case OAUpSlideDirection:
            newImageRect = newVisibleRect;
            newImageRect.origin.y += NSHeight(overlapRect);
            newImageRect.size.height -= NSHeight(overlapRect);
            break;
    ...
    //NSLog(@"newImageRect = %@", NSStringFromRect(newImageRect));
    
    newImageRep = [documentView bitmapForRect: newImageRect];
    //[[newImage TIFFRepresentation] writeToFile: @"/tmp/new.tiff" atomically: YES];
    //NSLog(@"newImage size = %@", NSStringFromSize([newImage size]));
    
    [documentView slideOutOldBitmapImageRep: oldImageRep newBitmapImageRep: newImageRep slideDirection: direction];
}
#endif
