...
RCS_ID("$Header: /Network/Source/CVS/OmniGroup/Frameworks/OmniAppKit/Widgets.subproj/OASearchField.m,v 1.16 2004/02/10 06:44:09 kc Exp $");
...
- (void)setSearchMode:(id)newSearchMode;
{
    ...
    itemCount = [items count];
    for (itemIndex = 0; itemIndex < itemCount; itemIndex++) {
        [[items objectAtIndex:itemIndex] setState:(itemIndex == selectedItemIndex)];
    }
...
- (void)controlTextDidChange:(NSNotification *)notification;
{
    if ([notification object] == searchField) {
        ...
        if (!flags.sendsWholeSearchString) {
            [self _destroyPartialStringActionDelayTimer];
            partialStringActionDelayTimer = [[NSTimer alloc] initWithFireDate:[NSDate dateWithTimeIntervalSinceNow:0.6] interval:0.0 target:self selector:@selector(_partialStringActionDelayTimerFired:) userInfo:nil repeats:NO];
...
- (void)_drawLeftImageForBounds:(NSRect)bounds;
{
    [OASearchFieldLeftImage drawInRect:[self _leftImageRectForBounds:bounds] fromRect:(NSRect){ { 0, 0 }, OASearchFieldLeftImageSize } operation:NSCompositeSourceOver fraction:1.0];
}
    ...
    if (menu != nil) {
        if (searchMode != nil) {
            ...
            if (searchModeIndex == -1)
                searchModeName = NSLocalizedStringFromTableInBundle(@"Search", @"OmniAppKit", [isa bundle], @"default search mode menu item");
            else
                searchModeName = [[menu itemAtIndex:searchModeIndex] title];
        } else
            searchModeName = NSLocalizedStringFromTableInBundle(@"Search", @"OmniAppKit", [isa bundle], @"default search mode menu item");
    } else
        searchModeName = NSLocalizedStringFromTableInBundle(@"Search", @"OmniAppKit", [isa bundle], @"default search mode menu item");
...
- (void)_chooseSearchMode:(id)sender;
{
    ...
    if ([delegate respondsToSelector:@selector(searchField:didChooseSearchMode:)])
