...
RCS_ID("$Header: /Network/Source/CVS/OmniGroup/Frameworks/OmniAppKit/Inspector.subproj/OAColorInspector.m,v 1.23 2004/02/10 04:07:32 kc Exp $");
...
{
    // This says 'image' to make it more clear that people can set background images via the color panel using the OAPatternColorPicker.
    return NSLocalizedStringFromTableInBundle(@"Color / Image", @"OmniAppKit", [OAColorInspector bundle], "window and menu title for Color inspector");
}
...
- (NSView *)inspectorView;
{
    if (!contentView) {
        ...
        // Allow applications to configure whether to show alpha w/o manually calling into NSColorPanel (and thus causing it to load its nib)
        if ([[OFPreference preferenceForKey:@"OAColorInspectorShowsAlpha"] boolValue])
