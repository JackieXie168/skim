...
RCS_ID("$Header: /Network/Source/CVS/OmniGroup/Frameworks/OmniFoundation/XML/OFXMLCursor.m,v 1.8 2004/02/10 04:07:48 kc Exp $");
...
/*.doc. Initializes the receiver so that -currentChild will return the root element of the document. */
{
    ...
    _OFXMLCursorStateInit(_state, [_document rootElement]);
...
- (BOOL)_checkInvariants;
{
    ...
    // The root element should always be in the stack.
    OBINVARIANT(_state[0].element == [_document rootElement]);
...
- (id)_nextChild:(BOOL)peek;
{
    ...
    OBINVARIANT([self _checkInvariants]);
    OBPOSTCONDITION(peek == (child != [self currentChild]));
    return child;
}
