...
RCS_ID("$Header: /Network/Source/CVS/OmniGroup/Frameworks/OmniFoundation/Tests/OFBTreeTest.m,v 1.13 2004/02/10 04:07:48 kc Exp $")
...
#define INSERT_COUNT 1000000
    ...
    permute(numbers, INSERT_COUNT);
    for (i = 0; i < INSERT_COUNT; i++) {
        should(OFBTreeFind(&btree, &numbers[i]));
    }
    ...
    for (i = 0; i < INSERT_COUNT; i++) {
        ...
        if (i > 0) {
            shouldnt(OFBTreeFind(&btree, &numbers[i-1]));
        }
        if (i+1 < INSERT_COUNT) {
            should(OFBTreeFind(&btree, &numbers[i+1]));
        }
    }
    ...
    permute(numbers, INSERT_COUNT);
    for (i = 0; i < INSERT_COUNT; i++) {
        shouldnt(OFBTreeFind(&btree, &numbers[i]));
    }
