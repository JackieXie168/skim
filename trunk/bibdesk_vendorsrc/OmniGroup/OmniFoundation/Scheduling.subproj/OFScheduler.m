...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2006-09-07/OmniGroup/Frameworks/OmniFoundation/Scheduling.subproj/OFScheduler.m 69639 2005-10-26 22:27:07Z wiml $")
...
- (BOOL)abortEvent:(OFScheduledEvent *)event;
{
	...
	[self scheduleEvents];
        ...
        index = [scheduleQueue indexOfObjectIdenticalTo:event inArraySortedUsingSelector:@selector(compare:)];
        if (index != NSNotFound) {
            wasFound = YES;
            [scheduleQueue removeObjectAtIndex:index];
        }
    }
    [scheduleLock unlock];
...
{
    return [self scheduleSelector:selector onObject:anObject atDate:[NSDate dateWithTimeIntervalSinceNow:time]];
}
...
{
    return [self scheduleSelector:selector onObject:anObject withBool:anArgument atDate:[NSDate dateWithTimeIntervalSinceNow:time]];
}
