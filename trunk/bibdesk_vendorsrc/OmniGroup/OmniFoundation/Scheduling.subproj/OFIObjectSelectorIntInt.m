...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2006-09-07/OmniGroup/Frameworks/OmniFoundation/Scheduling.subproj/OFIObjectSelectorIntInt.m 68913 2005-10-03 19:36:19Z kc $")
...
- (void)invoke;
{
    ...
    method = class_getInstanceMethod(((OFIObjectSelectorIntInt *)object)->isa,
				     selector);
    if (!method)
        [NSException raise:NSInvalidArgumentException format:@"%s(0x%x) does not respond to the selector %@", ((OFIObjectSelectorIntInt *)object)->isa->name, (unsigned)object, NSStringFromSelector(selector)];

    method->method_imp(object, selector, theInt, otherInt);
}
...
{
    OFIObjectSelectorIntInt *otherObject;
