...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2006-09-07/OmniGroup/Frameworks/OmniFoundation/OFRegularExpression.m 66170 2005-07-28 17:40:10Z kc $")
...
- (BOOL)findMatch:(OFRegularExpressionMatch *)match withScanner:(OFStringScanner *)scanner;
{
    if (matchStartsLine) {
        ...
        if (scannerScanLocation(scanner)) {
            ...
            atBeginningOfLine = (scannerReadCharacter(scanner) == '\n');
        } else {
            atBeginningOfLine = YES;
        }
        if (atBeginningOfLine && [self tryMatch:match withScanner:scanner atStartOfLine:YES])
            return YES;
