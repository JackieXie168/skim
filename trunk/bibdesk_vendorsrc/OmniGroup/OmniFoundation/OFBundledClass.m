...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2006-09-07/OmniGroup/Frameworks/OmniFoundation/OFBundledClass.m 68913 2005-10-03 19:36:19Z kc $")
...
+ (void)loadAllClasses;
{
    ...
    [bundleLock lock];
    NS_DURING {
        NSEnumerator *classNameEnumerator;
        NSString *aClassName;

        classNameEnumerator = [bundledClassRegistry keyEnumerator];
        while ((aClassName = [classNameEnumerator nextObject])) {
            ...
            NS_DURING {
                ...
                NSLog(@"+[OFBundledClass loadAllClasses]: Exception while loading %@: %@", aClassName, [localException reason]);
            } NS_ENDHANDLER;
        }
    } NS_HANDLER {
        NSLog(@"+[OFBundledClass loadAllClasses]: %@", [localException reason]);
    } NS_ENDHANDLER;
    [bundleLock unlock];
}
