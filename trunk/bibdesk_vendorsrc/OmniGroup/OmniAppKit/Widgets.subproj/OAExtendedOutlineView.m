...
- (void)setSmoothExpandCollapseEnabled:(BOOL)newSetting;
{
    flags.smoothExpandCollapse = newSetting;
}
...
- (void)expandItem:(id)item expandChildren:(BOOL)expandChildren;
{
    ...
    // Do the animation loop.  On each frame, display some percentage of the children pixels (starting from the bottom) and shift the following rows down by that amount.
...
- (NSArray *)selectedItemsWithoutSelectedParents;
{
    ...
    while ((selection = [enumerator nextObject])) {
        row = [selection intValue];
        ...
        if (level == -1)
            [result addObject:[self itemAtRow:[selection intValue]]];
    }
    return result;
}
