...
- (id)performDefaultImplementation {
	...
	BDSKFormatParser *parser = [BDSKFormatParser sharedParser];
	NSString *error = nil;
	
	if (![parser validateFormat:&formatString forField:field inFileType:BDSKBibtexString error:&error]) {
		[self setScriptErrorNumber:NSArgumentsWrongScriptError]; 
		[self setScriptErrorString:[NSString stringWithFormat:@"Invalid format string: %@", error]]; 
		return nil;
	}
	
	NSString *string = [parser parseFormat:formatString forField:field ofItem:pub];
	
	if ([[[OFPreferenceWrapper sharedPreferenceWrapper] stringArrayForKey:BDSKLocalFileFieldsKey] containsObject:field]) {
		NSString *papersFolderPath = [[[OFPreferenceWrapper sharedPreferenceWrapper] stringForKey:BDSKPapersFolderPathKey] stringByExpandingTildeInPath];
